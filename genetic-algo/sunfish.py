...
class Searcher:
    ...
    def bound(self, pos, gamma, depth, pst, root=True):
        """ returns r where
                s(pos) <= r < gamma    if gamma > s(pos)
                gamma <= r <= s(pos)   if gamma <= s(pos)"""
        self.nodes += 1
        # print(f'Inside the bound function with depth: {depth}')
        ...
        def moves(pst):
            ...
            if depth > 0 and not root and any(c in pos.board for c in 'RBNQ'):
                yield None, -self.bound(pos.nullmove(), 1-gamma, depth-3, pst, root=False)
            ...
            if killer and (depth > 0 or pos.value(killer, pst) >= QS_LIMIT):
                yield killer, -self.bound(pos.move(killer, pst), 1-gamma, depth-1, pst, root=False)
            # Then all the other moves
            for move in pos.gen_moves():
                if depth > 0 or pos.value(move,pst) >= QS_LIMIT:
                    yield move, -self.bound(pos.move(move, pst), 1-gamma, depth-1, pst, root=False)

        # Run through the moves, shortcutting when possible
        best = -MATE_UPPER
        for move, score in moves(pst):
            best = max(best, score)
            if best >= gamma:
        ...
        if best < gamma and best < 0 and depth > 0:
            is_dead = lambda pos: any(pos.value(m, pst) >= MATE_LOWER for m in pos.gen_moves())
            if all(is_dead(pos.move(m, pst)) for m in pos.gen_moves()):
    ...
    def _search(self, pos, pst):
        ...
        for depth in range(1, 4):
            ...
            while lower < upper - EVAL_ROUGHNESS:
                ...
                score = self.bound(pos, gamma, depth, pst)
                if score >= gamma:
                    lower = score
                if score < gamma:
                    upper = score
            ...
            score = self.bound(pos, lower, depth, pst)

            # Yield so the user may inspect the search
            yield

    def search(self, pos, secs, pst):
        start = time.time()
        print("searcher called")
        for _ in self._search(pos, pst):
            if time.time() - start > secs:
                break
        # If the game hasn't finished we can retrieve our move from the
        # transposition table.
        print("returning a move")
        print(f'TP Score: {self.tp_score.get((pos, self.depth, True))}')
        return self.tp_move.get(pos)
