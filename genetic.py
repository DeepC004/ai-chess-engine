...
class GeneticAlgorithm():
    ...
    def __init__(self,task,fromgeneration=1):
        ...
        if task=="trainnew":
            ...
            print('Creating population of size 64')
            self.population = self.generateRandomGeneration(64)
            self.run()
        elif task=="train":
    ...
    def selfPlay(self,pstwhite,pstblack):
        ...
        for d in range(startindex,20):
            ...
            m = sunfish.Searcher().search(pos, secs, pstCurrent) #Modified Sunfish searcher that takes in variable piece tables
    ...
    def run(self):
        eval_generation = {}
        eval_generation['1'] = self.population[0]
        ...
        while True:
            ...
            pairs = []
            for newpop in newgeneration:
                pairs.append(self.padSolution(newpop))
            pairs = self.helper.formPairs(pairs)
            self.generationcount += 1
            eval_generation[f'{self.generationcount}'] = newgeneration[0]

        with open('all_generations', 'wb') as f:
            pickle.dump(eval_generation, f)
            f.close()
